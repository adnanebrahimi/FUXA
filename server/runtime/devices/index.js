...
function stop() {
  ...
  return new Promise(function (resolve, reject) {
    ...
    return Promise.all(deviceStopfnc).then(
      (values) => {
        resolve(true);
        wokingStatus = null;
      },
    ...
    activeDevices[device.id]
      ...
      .catch(function (err) {
        runtime.logger.error(
          "devices.update-device " + device.name + ": " + err
        );
      });
  }
}
  ...
  if (!activeDevices[device.id]) {
    ...
    activeDevices[device.id]
      ...
      .catch(function (err) {
        delete activeDevices[device.id];
        runtime.logger.error(
          "devices.remove-device by stop " + device.name + ": " + err
        );
      });
  }
}
...
function load() {
  ...
  for (var id in activeDevices) {
    if (Object.keys(tempdevices).indexOf(id) < 0) {
      runtime.logger.info(
        `devices.load-removed: '${activeDevices[id].name}'`,
        true
      );
    }
  }
...
function loadDevice(device) {
  if (activeDevices[device.id]) {
    ...
    if (tdev && tdev.start) {
      ...
      activeDevices[device.id].bindGetProperty(
        runtime.project.getDeviceProperty
      );
      activeDevices[device.id].bindUpdateConnectionStatus(
        setDeviceConnectionStatus
      );
    } else {
      if (!Device.isInternal(device)) {
        runtime.logger.warn(
          "try to create " + device.name + " but plugin is missing!"
        );
      }
      return false;
    }
  ...
  if (runtime.settings.daqEnabled) {
    var fncToSaveDaqValue = runtime.daqStorage.addDaqNode(
      device.id,
      activeDevices[device.id].getTagProperty
    );
    activeDevices[device.id].bindSaveDaqValue(fncToSaveDaqValue);
    activeDevices[device.id].bindGetDaqValueToRestore(
      runtime.daqStorage.getCurrentStorageFnc()
    );
  }
  return true;
}
  ...
  try {
    ...
    for (var id in devices) {
      if (!deviceName || devices[id].name === deviceName) {
        const tag = Object.values(devices[id].tags).find(
...
function getDevice(deviceName, asInterface) {
  try {
    ...
    if (device) {
      return asInterface
        ? activeDevices[device.id].getComm()
        : activeDevices[device.id];
    }
    return null;
  } catch (err) {
